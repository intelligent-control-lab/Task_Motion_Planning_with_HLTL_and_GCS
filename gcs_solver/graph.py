...
from gcs_solver.convex_sets import ConvexSet
from gcs_solver.convex_functions import ConvexFunction, Constant
...
class GraphOfConvexSets():
    ...
    def add_edge(self, u, v, length=None):
        ...
        self.lengths[(u, v)] = length      # lengths is edges!!!
    ...
    def graphviz(self, vertex_labels=None, edge_labels=None, valid_edge =None, w_var = None):
        ...
        for label in vertex_labels:
            ...
            # else: 
            #     if 'target' in label:
            #         G.node(str(label) + np.array2string(w_var[label]))
            #     else:
            #         G.node(str(label))
        for k, edge in enumerate(self.edges):
            ...
            if valid_edge is None:
                ...
                if k in valid_edge:
                    G.edge(str(u), str(v), str(np.array2string(w_var[u])), color='red')
                else:
                    G.edge(str(u), str(v), str(edge_labels[k]))
        # ipdb.set_trace()
        return G
