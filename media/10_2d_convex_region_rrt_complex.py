...
q_goal_signle = np.array([9.5,1.5])
...
obstacle = plt.Rectangle((4, 2), 2, 1, color='red', alpha=0.5,angle=0)
ax.add_patch(obstacle)
obstacle = plt.Rectangle((7, 1), 2, 1, color='red', alpha=0.5,angle=0)
ax.add_patch(obstacle)
...
if Draw_baseline == True:
    ...
    for t in range(len(path)):
        ax.plot(path[t][0],path[t][1], 'ko', markersize=3, linewidth=2)  # 'bo' means blue color, circle marker
...
color = ['Blues','Reds','Oranges','Purples']
for i in range(len(hpoly_list)):
    hpoly = hpoly_list[i]
    A = hpoly.A()
    b = hpoly.b()
    ...
    feasible_region = np.all(constraints, axis=0)
    # Plot the feasible region
    ax.imshow(feasible_region, extent=(x.min(), x.max(), y.min(), y.max()), origin="lower", cmap=color[i], alpha=0.3)
...
# Save the plot as a PNG file
plt.savefig('../media/feasible_region1.png')
print("done")