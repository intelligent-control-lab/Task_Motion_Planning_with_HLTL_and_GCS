...
class TransitionSystem(DirectedGraph):
    ...
    def findConnectRegion(self,  label):   
        ...
        replaced_string = [item.replace('target', 'connect') for sublist in label for item in sublist if item]

        row_number = -1
        for idx, row_idx in enumerate(self.target_connection_labels):
            if replaced_string in row_idx:
                row_number = idx
                break
            if row_number != -1:
                break
    ...
    def AddEdgewithConnectionToInit(self):
        ...
        #     connection_index = self.findConnectRegion(self.target_labels[target_vertex])
        #     connection_vertex_index = self.v_idx
        #     self.vertices.append(connection_vertex_index)
        #     self.partitions[connection_vertex_index] = self.target_connection_vertices_set[connection_index]
        #     self.labels[connection_vertex_index] = f'{self.target_connection_labels[connection_index]}{self.num_connection}'
        #     self.v_idx += 1
        #     self.num_connection += 1
            
        #     # edges with init to connection edges
        #     edge = (init_vertex_index, connection_vertex_index)
        #     assert edge not in self.edges, "edge already exists!"
        #     self.edges.append(edge)
            
        #     # edges with connection to target edges
        #     edge = (connection_vertex_index, target_vertex)
        #     assert edge not in self.edges, "edge already exists!"
        #     self.edges.append(edge)
        
        # #new method which 
        # for v in self.init_connect_vertice:
        #     edge = (init_vertex_index, v)
        #     assert edge not in self.edges, "edge already exists!"
        #     self.edges.append(edge) 
