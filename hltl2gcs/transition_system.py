...
class TransitionSystem(DirectedGraph):
    ...
    def Product(self, fa, start_point, order, continuity, is_handover, regions_in_paris, args):
        ...
        for v in prod_vertices:
            for v_prime in prod_vertices:
                ...
                if isinstance(ts_label, str): # TODO connecting nodes are labelled with string
                    if 'target' in ts_label or 'obstacle' in ts_label:   # add connect ???
                        index = ts_label.rfind(']') + 1
                        cleaned_s = ts_label[:index]
                        ts_label = ast.literal_eval(cleaned_s)
                        sat = self.ts_label_satisfies_fa_label(ts_label, fa_label)
                ...
                # Any ts label should not fasify any fa label
                if sat:
                    sat = not self.conflict_with_any_fa_label(ts_label, fa_label)
                if sat:
    ...
    @staticmethod
    def conflict_with_any_fa_label(ts_labels, fa_labels):
            for fa_label in fa_labels:
                ...
                for ts_label in ts_labels:
                    substitutions = dict()
                    try:
                        expression = parse_expr(str(fa_label))
                    except:
                        print(fa_label)
                    for symbol in expression.free_symbols:
                        if any([str(symbol) in ts_l for ts_l in ts_label]):
                            substitutions[symbol] = True
                    expr_with_subs = fa_label.subs(substitutions)
                    if expr_with_subs == False:
                        return True
            return False
    ...
    def satisfies(ts_label, fa_label):
        ...
        for symbol in expression.free_symbols:
            if any([label.strip() != '' and (label in str(symbol) or str(symbol) in label) for label in ts_label]):
        ...
        has_obstacle = False
        # if any(['obstacle' in label for label in ts_label]) and any(['obstacle' in str(symbol) for symbol in parse_expr(fa_label).free_symbols]):
        #     print(ts_label, fa_label, res)
        return res

    def ts_label_satisfies_fa_label(self, ts_labels:list, fa_labels:list):
        ...
        if self.num_robot >= len(fa_labels):
            if len(ts_labels) < len(fa_labels):
                ts_labels = ts_labels + [''] * (self.num_robot - len(ts_labels))
            ...
            for permutations_fa_label in permutations_fa_labels:
                for selected_ts_label in selected_ts_labels:
                    ...
                    for ts_label, fa_label in ts_fa_combination:
                        if fa_label != to_dnf("True") and not self.satisfies(ts_label, str(fa_label)):
                            satisfy = False
                            break
                    if satisfy:
                        # try:
                        #     if any(['obstacle' in l for label in ts_labels for  l in label]) and \
                        #     any(['obstacle' in str(symbol) for fa_label in fa_labels for symbol in parse_expr(str(fa_label)).free_symbols]) and \
                        #     any(['target_1_place_object_3' for fa_label in fa_labels for symbol in parse_expr(str(fa_label)).free_symbols]) and  \
                        #     not any(['target_4_place_object_1' in str(symbol)  for fa_label in fa_labels for symbol in parse_expr(str(fa_label)).free_symbols])  and \
                        #     not any(['target_4_place_object_3' in str(symbol)  for fa_label in fa_labels for symbol in parse_expr(str(fa_label)).free_symbols])  \
                        #         :
                        #         print(ts_labels, fa_labels, satisfy)
                ...
                for permutations_fa_label in permutations_selected_fa_label:
                    ...
                    if satisfy:
                        ...
                        for fa_label in unexplored_fa_labels:
                            if fa_label != to_dnf("True") and not self.satisfies([""], str(fa_label)):
                                satisfy = False
                                break
                        if satisfy:
                            return True
