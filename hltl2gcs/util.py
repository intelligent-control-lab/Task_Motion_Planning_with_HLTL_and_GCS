...
def vis_graph(graph, title, latex=False, buchi_graph=False):
    ...
    if buchi_graph:
        for node in graph_copy.nodes():
            graph_copy.add_edge(node, node, label=graph_copy.nodes[node]['label'])
            graph_copy.nodes[node]['label'] =  graph_copy.nodes[node]['name']
    if latex:
        for edge in graph_copy.edges():
            label = graph_copy.edges[edge]['label']
            label_latex = str(label).replace('~', '!').replace('True', '\\top').replace('&', '\\ \\&\\ ').\
                replace('init', '\\text{init}').replace('accept', '\\text{accept}').replace('_', '')
            graph_copy.edges[edge]['label'] = label_latex
        for node in graph_copy.nodes():
            label = graph_copy.nodes[node]['label']
            label_latex = str(label).replace('~', '!').replace('True', '\\top').replace('&', '\\ \\&\\ ').\
                replace('init', '\\text{init}').replace('accept', '\\text{accept}').replace('_', '')
            del graph_copy.nodes[node]['name']
            graph_copy.nodes[node]['label'] = label_latex
...
def create_parser():
    ...
    parser.add_argument('--debug', action='store_true', help='Enable print search to terminal')
    parser.add_argument('--draw', action='store_true', help='Enable dot graph')
    return parser
