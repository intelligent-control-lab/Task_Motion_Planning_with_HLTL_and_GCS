...
spec200 = "(F (target_5_pick_object_2 & F (target_6_place_object_2)))"
spec300 = "(F (target_1_pick_object_2 & F (target_4_place_object_2)))" 
is_handover = True     
...
if args.case == -1:
    hierarchy = []
    level_one = dict()
    level_one["p0"] = "F (p100 & F (p200 | p300))"
...
if SHOW_ROBOT == True:
    ...
    q_object2_init = RigidTransform2Array(RigidTransform(RollPitchYaw(-np.pi/2,np.pi/2,0).ToRotationMatrix(),[0.5, -0.52, 0.1]))
    q_object3_init = RigidTransform2Array(RigidTransform(RollPitchYaw(-np.pi/2,np.pi/2,0).ToRotationMatrix(),[0, -0.52, 0.1]))
    ...
    file_path = "../media/four_iiwa_rectangular_or_case2.html"
